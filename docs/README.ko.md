...
문서만 읽어야 하는 개념 저장소가 아닙니다. 실제 라이트블링거 애플리케이션에 저작권 문제가 없는 공개용 프로젝트 **ORBITAL ECHO**를 넣고, 승인 대본부터 구간·샷, 연속성·에셋, 프롬프트 정본, 스토리보드 결과 이력까지 실행한 화면을 공개했습니다.

![라이트블링거 구간 탐색기와 샷 타이밍](../assets/demo/02-shot-design.png)

| 대본 | 구간 | 샷 | 동기화 에셋 | 완료 스토리보드 | 실패 단계 |
| ---: | ---: | ---: | ---: | ---: | ---: |
| 42초 | 2개 | 6개 | 5개 | 3개 | 0개 |
...
공개 데모는 실제 앱의 UI, API 계약, 데이터 구조, 승인 게이트, 에셋 연결, 프롬프트 및 렌더 이력 화면을 그대로 사용합니다. 고객 작품이나 비밀정보를 노출하지 않고 반복 확인할 수 있도록 결과값만 격리된 고정 데이터로 구성했습니다.
